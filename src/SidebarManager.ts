...
export class SidebarManager {
  ...
  public updateChatList = async (): Promise<void> => {
    ...
    try {
        ...
        if (currentActiveChatId) { /* ... визначення предків ... */ }

        // --- ЛОГУЄМО ПОВНУ СТРУКТУРУ ---
        try {
             this.plugin.logger.debug(`[Update #${currentUpdateId}] Hierarchy data received (${hierarchy.length} root items):`, JSON.parse(JSON.stringify(hierarchy))); // Глибоке копіювання для логування
        } catch (e) {
             this.plugin.logger.error("Could not stringify hierarchy for logging", e);
             console.log(hierarchy); // Виводимо як об'єкт, якщо JSON не працює
        }
        // ---

        if (hierarchy.length === 0) { container.createDiv({ cls: "menu-info-text", text: "No saved chats or folders yet." }); }
        else { hierarchy.forEach(node => this.renderHierarchyNode(node, container, 0, currentActiveChatId, activeAncestorPaths, currentUpdateId)); } // Передаємо ID оновлення
  ...
  private renderHierarchyNode(
    node: HierarchyNode,
    parentElement: HTMLElement, // Тепер це або root контейнер, або childrenContainer батька
    level: number,
    ...
    // Логуємо батьківський елемент, куди додаємо поточний вузол
    ...
    if (node.type === 'folder') {
        ...
        // --- ПОВЕРНУЛИ РЕКУРСИВНИЙ ВИКЛИК ---
        if (node.children && node.children.length > 0) { // Додано перевірку наявності children
