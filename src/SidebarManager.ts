...
export class SidebarManager {
    ...
    private attachSidebarEventListeners(): void {
        ...
        this.plugin.registerDomEvent(this.newChatSidebarButton, "click", e => { e.stopPropagation(); this.handleNewChatClick(); });
    ...
    public updateChatList = async (): Promise<void> => {
        ...
        if (!isVisible) { this.plugin.logger.debug("[SidebarManager.updateChatList] Section collapsed, updating content only."); }
        else { this.plugin.logger.debug("[SidebarManager.updateChatList] Updating chat list content..."); }
        ...
        try {
            ...
            if (chats.length === 0) { container.createDiv({ cls: "menu-info-text", text: "No saved chats yet." }); }
            else {
                chats.forEach(chatMeta => {
                    ...
                    if (dateText === "Invalid date") { this.plugin.logger.warn(`[SidebarManager.updateChatList] Invalid date for chat ${chatMeta.id}`); }
                    ...
                    this.plugin.registerDomEvent(chatOptionEl, "click", async e => { if (!(e.target instanceof Element && e.target.closest(`.${CSS_CHAT_ITEM_OPTIONS}`))) { if (chatMeta.id !== this.plugin.chatManager?.getActiveChatId()) { await this.plugin.chatManager.setActiveChat(chatMeta.id); } } });
                    this.plugin.registerDomEvent(optionsBtn, "click", e => { e.stopPropagation(); this.showChatContextMenu(e, chatMeta); });
                    this.plugin.registerDomEvent(chatOptionEl, "contextmenu", e => { this.showChatContextMenu(e, chatMeta); });
                });
            }
            ...
            container.empty(); container.createDiv({ text: "Error loading chats.", cls: "menu-error-text" });
    ...
    public updateRoleList = async (): Promise<void> => {
        ...
        if (!isVisible) { this.plugin.logger.debug("[SidebarManager.updateRoleList] Section collapsed, updating content only."); }
        else { this.plugin.logger.debug("[SidebarManager.updateRoleList] Updating role list content..."); }
        ...
        try {
            ...
            container.empty(); container.createDiv({ text: "Error loading roles.", cls: "menu-error-text" });
             // Висота керується CSS
        } finally { requestAnimationFrame(() => { if (container?.isConnected) { container.scrollTop = currentScrollTop; } }); }
    };
    ...
    private handleRolePanelItemClick = async (
        ...
        if (newRolePath !== normalizedCurrentRolePath) {
            const activeChat = await this.plugin.chatManager?.getActiveChat();
            try {
                if (activeChat) { await this.plugin.chatManager.updateActiveChatMetadata({ selectedRolePath: newRolePath }); }
                else { this.plugin.settings.selectedRolePath = newRolePath; await this.plugin.saveSettings(); this.plugin.emit("role-changed", roleNameForEvent); this.plugin.promptService?.clearRoleCache?.(); }
    ...
    private async toggleSection(clickedHeaderEl: HTMLElement): Promise<void> {
        ...
        if (sectionType === "chats") { [contentEl, updateFunction, otherHeaderEl, otherContentEl, otherSectionType] = [this.chatPanelListEl, this.updateChatList, this.rolePanelHeaderEl, this.rolePanelListEl, "roles"]; }
        else if (sectionType === "roles") { [contentEl, updateFunction, otherHeaderEl, otherContentEl, otherSectionType] = [this.rolePanelListEl, this.updateRoleList, this.chatPanelHeaderEl, this.chatPanelListEl, "chats"]; }

        if (!contentEl || !iconEl || !updateFunction || !otherHeaderEl || !otherContentEl || !otherSectionType) { this.plugin.logger.error("Sidebar toggle elements missing:", sectionType); return; }
        ...
        if (isCurrentlyCollapsed) {
            ...
            if (otherHeaderEl.getAttribute("data-collapsed") === "false") {
                ...
                otherHeaderEl.setAttribute("data-collapsed", "true"); if (otherIconEl) setIcon(otherIconEl, COLLAPSE_ICON);
            ...
            clickedHeaderEl.setAttribute("data-collapsed", "false"); setIcon(iconEl, EXPAND_ICON);
            ...
            clickedHeaderEl.setAttribute("data-collapsed", "true"); setIcon(iconEl, COLLAPSE_ICON);
    ...
    private handleNewChatClick = async (): Promise<void> => {
        ...
        try {
            ...
            if (newChat) { new Notice(`Created new chat: ${newChat.metadata.name}`); this.plugin.emit('focus-input-request'); }
            else { new Notice("Failed to create new chat."); }
        } catch (error) { this.plugin.logger.error("[SidebarManager] Error creating new chat:", error); new Notice("Error creating new chat."); }
    };
     ...
     private async handleContextMenuClone(chatId: string): Promise<void> {
        ...
        const notice = new Notice("Cloning chat...", 0); try { const c = await this.plugin.chatManager.cloneChat(chatId); if(c) { new Notice(`Chat cloned as "${c.metadata.name}"`); this.plugin.emit('focus-input-request'); } } catch(e) { this.plugin.logger.error(`Clone error:`, e); } finally { notice.hide(); }
    }
    ...
    private async handleContextMenuRename(chatId: string, currentName: string): Promise<void> {
        ...
        new PromptModal(this.app, "Rename Chat", `New name for "${currentName}":`, currentName, async (n) => { const t = n?.trim(); if (t && t !== "" && t !== currentName) { const s = await this.plugin.chatManager.renameChat(chatId, t); new Notice(s ? `Renamed to "${t}"` : `Failed to rename.`); } else { new Notice(t === currentName ? `Name unchanged.` : `Rename cancelled.`); } this.plugin.emit('focus-input-request'); }).open();
    }
    ...
    private async exportSpecificChat(chatId: string): Promise<void> {
        ...
        const notice = new Notice(`Exporting chat...`, 0); try { const chat = await this.plugin.chatManager.getChat(chatId); if (!chat || chat.messages.length === 0) { new Notice("Chat empty/not found."); return; } const md = this.formatChatToMarkdown(chat.messages, chat.metadata); const ts = new Date().toISOString().replace(/[:.]/g, "-"); const safeName = chat.metadata.name.replace(/[\\/?:*"<>|]/g, "-"); const filename = `ollama-chat-${safeName}-${ts}.md`; let fPath = this.plugin.settings.chatExportFolderPath?.trim(); let fFolder: TFolder | null = null; if (fPath) { fPath = normalizePath(fPath); const af = this.app.vault.getAbstractFileByPath(fPath); if (!af) { try { await this.app.vault.createFolder(fPath); fFolder = this.app.vault.getAbstractFileByPath(fPath) as TFolder; if (fFolder) new Notice(`Created folder: ${fPath}`); } catch (err) { this.plugin.logger.error("Folder creation error:", err); new Notice(`Folder error. Saving to root.`); fFolder = this.app.vault.getRoot(); } } else if (af instanceof TFolder) { fFolder = af; } else { new Notice(`Path not folder. Saving to root.`); fFolder = this.app.vault.getRoot(); } } else { fFolder = this.app.vault.getRoot(); } if (!fFolder) { this.plugin.logger.error("Target folder error."); new Notice("Export folder error."); return; } const filePath = normalizePath(`${fFolder.path}/${filename}`); const file = await this.app.vault.create(filePath, md); new Notice(`Exported to ${file.path}`); } catch (e) { this.plugin.logger.error(`Export error:`, e); new Notice("Export failed."); } finally { notice.hide(); }
    }
    ...
    private async handleContextMenuClear(chatId: string, chatName: string): Promise<void> {
        ...
        new ConfirmModal(this.app, "Clear Messages", `Clear messages in "${chatName}"?`, async () => { const n = new Notice("Clearing...", 0); try { const s = await this.plugin.chatManager.clearChatMessagesById(chatId); new Notice(s ? `Messages cleared.` : `Failed to clear.`); } catch (e) { this.plugin.logger.error(`Clear error:`, e); new Notice("Clear failed."); } finally { n.hide(); } }).open();
    }
    ...
    private async handleContextMenuDelete(chatId: string, chatName: string): Promise<void> {
        ...
        new ConfirmModal(this.app, "Delete Chat", `Delete chat "${chatName}"?`, async () => { const n = new Notice("Deleting...", 0); try { const s = await this.plugin.chatManager.deleteChat(chatId); if(s) new Notice(`Chat deleted.`); } catch (e) { this.plugin.logger.error(`Delete error:`, e); new Notice("Delete failed."); } finally { n.hide(); } }).open();
    }
     ...
     private formatChatToMarkdown(messagesToFormat: Message[], metadata: ChatMetadata): string {
        ...
        messagesToFormat.forEach(message => {
            ...
            if (message.role === "assistant") {
                ...
                } catch (e) { this.plugin.logger.warn("Error processing assistant content for export:", e); }
                if (!content) return;
            }
    ...
    private formatDateSeparator(date: Date): string {
        ...
        if (this.isSameDay(date, now)) return "Today"; if (this.isSameDay(date, y)) return "Yesterday";
    ...
    private formatRelativeDate(date: Date): string {
        ...
        const now = new Date(); const diffSeconds = Math.round((now.getTime() - date.getTime()) / 1000);
        if (diffSeconds < 5) return "Just now"; if (diffSeconds < 60) return `${diffSeconds} sec ago`;
        const diffMinutes = Math.floor(diffSeconds / 60); if (diffMinutes < 60) return `${diffMinutes} min ago`;
        const diffHours = Math.floor(diffMinutes / 60); if (diffHours < 24) return `${diffHours} hr ago`;
        const diffDays = Math.floor(diffHours / 24); if (diffDays === 1) return "Yesterday"; if (diffDays < 7) return `${diffDays} days ago`;
