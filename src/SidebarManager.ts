...
export class SidebarManager {
  ...
  public updateChatList = async (): Promise<void> => {
    this.updateCounter++; const currentUpdateId = this.updateCounter; // Лічильник для логів
  ...
  private renderHierarchyNode(
      ...
      if (node.type === 'folder') {
          ...
          if (node.children.length > 0) {
              node.children.forEach(childNode => this.renderHierarchyNode(childNode, childrenContainer, level + 1, activeChatId, activeAncestorPaths, updateId)); // Передаємо updateId далі
          }
  ...
  private handleToggleFolder(folderPath: string): void {
      ...
      if (folderIconEl) { setIcon(folderIconEl, newState ? FOLDER_ICON_OPEN : FOLDER_ICON_CLOSED); } // Змінює іконку
  }
