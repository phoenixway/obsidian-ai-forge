...
export class SidebarManager {
  ...
  public updateChatList = async (): Promise<void> => {
    this.updateCounter++; const currentUpdateId = this.updateCounter;
    const container = this.chatPanelListContainerEl;
    if (!container || !this.plugin.chatManager) { this.plugin.logger.debug(`[Update #${currentUpdateId}] Skipping: Container/Manager missing.`); return; }
    ...
    try {
        ...
        else { hierarchy.forEach(node => this.renderHierarchyNode(node, container, 0, currentActiveChatId, activeAncestorPaths, currentUpdateId)); }
        this.plugin.logger.info(`[Update #${currentUpdateId}] <<<<< FINISHED updateChatList (rendering done)`);
    } catch (error) { this.plugin.logger.error(`[Update #${currentUpdateId}] Error rendering hierarchy:`, error); container.empty(); container.createDiv({ text: "Error loading chat structure.", cls: "menu-error-text" });
    } finally { container.classList.remove("is-loading"); requestAnimationFrame(() => { if (container?.isConnected) { container.scrollTop = currentScrollTop; } }); this.plugin.logger.debug(`[Update #${currentUpdateId}] Finally block executed, loading class removed.`); }
  };


  private renderHierarchyNode(
      node: HierarchyNode, parentElement: HTMLElement, level: number,
      activeChatId: string | null, activeAncestorPaths: Set<string>, updateId: number
  ): void {
      const itemEl = parentElement.createDiv({ cls: [CSS_HIERARCHY_ITEM, `${CSS_HIERARCHY_INDENT_PREFIX}${level}`] });
      if (node.type === 'folder') { itemEl.dataset.path = node.path; }
      ...
      if (node.type === 'folder') {
          ...
          const folderIcon = itemContentEl.createSpan({ cls: CSS_FOLDER_ICON }); setIcon(folderIcon, isExpanded ? FOLDER_ICON_OPEN : FOLDER_ICON_CLOSED);
          itemContentEl.createSpan({ cls: CSS_HIERARCHY_ITEM_TEXT, text: node.name });
          const optionsBtn = itemContentEl.createEl("button", { cls: [CSS_HIERARCHY_ITEM_OPTIONS, "clickable-icon"], attr: { "aria-label": "Folder options", title: "More options" }, }); setIcon(optionsBtn, "lucide-more-horizontal");
          ...
          if (node.children && node.children.length > 0) { node.children.forEach(childNode => this.renderHierarchyNode(childNode, childrenContainer, level + 1, activeChatId, activeAncestorPaths, updateId)); }
      } else if (node.type === 'chat') {
          itemEl.addClass(CSS_CHAT_ITEM); const chatMeta = node.metadata; const isActive = chatMeta.id === activeChatId; if (isActive) { itemEl.addClass(CSS_ROLE_PANEL_ITEM_ACTIVE); }
          const chatIcon = itemContentEl.createSpan({ cls: CSS_FOLDER_ICON }); setIcon(chatIcon, isActive ? CHAT_ICON_ACTIVE : CHAT_ICON);
          itemContentEl.createSpan({ cls: CSS_HIERARCHY_ITEM_TEXT, text: chatMeta.name });
          const detailsWrapper = itemContentEl.createDiv({cls: CSS_CHAT_ITEM_DETAILS}); try { const lastModifiedDate = new Date(chatMeta.lastModified); const dateText = !isNaN(lastModifiedDate.getTime()) ? this.formatRelativeDate(lastModifiedDate) : "Invalid date"; if (dateText === "Invalid date") { this.plugin.logger.warn(`Invalid date for chat ${chatMeta.id}`); } detailsWrapper.createDiv({ cls: CSS_CHAT_ITEM_DATE, text: dateText }); } catch(e) { this.plugin.logger.error(`Error formatting date for chat ${chatMeta.id}: `, e); detailsWrapper.createDiv({ cls: CSS_CHAT_ITEM_DATE, text: "Date error" }); }
          const optionsBtn = itemContentEl.createEl("button", { cls: [CSS_HIERARCHY_ITEM_OPTIONS, "clickable-icon"], attr: { "aria-label": "Chat options", title: "More options" }, }); setIcon(optionsBtn, "lucide-more-horizontal");
  ...
  private async toggleSection(clickedHeaderEl: HTMLElement): Promise<void> {
    ...
    if (sectionType === "chats") {
        contentEl = chatContent; updateFunction = this.updateChatList;
        otherHeaderEl = roleHeader; otherContentEl = roleContent; otherSectionType = "roles";
    } else {
        contentEl = roleContent; updateFunction = this.updateRoleList;
        otherHeaderEl = chatHeader; otherContentEl = chatContent; otherSectionType = "chats";
    }

    if (!contentEl || !iconEl || !updateFunction || !otherHeaderEl || !otherContentEl || !otherSectionType) {
        this.plugin.logger.error("Could not find all required elements for sidebar accordion toggle:", sectionType); return;
    }
    const boundUpdateFunction = updateFunction.bind(this);

    if (isCurrentlyCollapsed) { // Розгортаємо поточну
        // 1. Згортаємо ІНШУ
        if (otherHeaderEl.getAttribute("data-collapsed") === "false") {
            ...
            if (otherIconEl) setIcon(otherIconEl, COLLAPSE_ICON_ACCORDION); // Згорнута іконка
            otherContentEl.classList.remove(CSS_EXPANDED_CLASS);
            otherContentEl.classList.add(CSS_SIDEBAR_SECTION_CONTENT_HIDDEN); // Швидко приховуємо
            const otherHeaderActionsEl = otherHeaderEl.querySelector<HTMLElement>(`.${CSS_SIDEBAR_HEADER_ACTIONS}`);
            if(otherHeaderActionsEl && otherIconEl) otherHeaderActionsEl.insertBefore(otherIconEl, otherHeaderActionsEl.firstChild); // Переміщаємо шеврон перед кнопками
            if(otherHeaderActionsEl) otherHeaderActionsEl.style.display = 'none'; // Ховаємо дії іншої секції
        }
        // 2. Розгортаємо поточну
        clickedHeaderEl.setAttribute("data-collapsed", "false");
        setIcon(iconEl, EXPAND_ICON_ACCORDION); // Розгорнута іконка
        ...
        if(headerActionsEl && iconEl) headerActionsEl.insertBefore(iconEl, headerActionsEl.firstChild); // Переміщаємо шеврон
        if(headerActionsEl) headerActionsEl.style.display = ''; // Показуємо дії

        try {
            await boundUpdateFunction();
             requestAnimationFrame(() => { if (contentEl?.isConnected && clickedHeaderEl.getAttribute("data-collapsed") === "false") { contentEl.classList.add(CSS_EXPANDED_CLASS); this.plugin.logger.debug(`Expanding sidebar section: ${sectionType}`); } });
        } catch (error) {
             this.plugin.logger.error(`Error updating sidebar section ${sectionType}:`, error);
             contentEl.setText(`Error loading ${sectionType}.`);
             requestAnimationFrame(() => { if (contentEl?.isConnected && clickedHeaderEl.getAttribute("data-collapsed") === "false") { contentEl.classList.add(CSS_EXPANDED_CLASS); } });
        }

    } else { // Згортаємо поточну
        this.plugin.logger.debug(`Collapsing sidebar section: ${sectionType}`);
        clickedHeaderEl.setAttribute("data-collapsed", "true");
        setIcon(iconEl, COLLAPSE_ICON_ACCORDION); // Згорнута іконка
