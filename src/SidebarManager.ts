...
export class SidebarManager {
  ...
  public updateChatList = async (): Promise<void> => {
    ...
    container.empty(); // Очищення контейнера
    ...
    try {
      ...
      // Логування отриманої ієрархії (тільки імена для читабельності)
      ...
      // Визначення предків активного чату
      if (currentActiveChatId) {
        /* ... логіка визначення предків ... */
      }
      ...
      container.empty(); // Очистити у разі помилки
  ...
  private renderHierarchyNode(
    ...
    updateId: number // Додано ID для логування
  ): void {
    const nodeName = node.type === "folder" ? node.name : node.metadata.name;
    // Логуємо початок рендерингу КОЖНОГО вузла
    this.plugin.logger.debug(
    ...
    if (node.type === "folder") {
      ...
      itemEl.createDiv({ cls: CSS_HIERARCHY_ITEM_CHILDREN }); // Поки що не заповнюємо
    } else {
