...
export interface OllamaPluginSettings extends LoggerSettings {
  ...
  maxDocumentContentLengthForLlm: number; // <--- НОВЕ ПОЛЕ
...
export const DEFAULT_SETTINGS: OllamaPluginSettings = {
  ...
  maxDocumentContentLengthForLlm: 4000, // <--- ЗНАЧЕННЯ ЗА ЗАМОВЧУВАННЯМ
...
export class OllamaSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Allow AI models to use registered tools/agents to perform actions. Requires compatible models (e.g., Llama 3.1, some Mistral models)."
      )
      .addToggle(toggle =>
        toggle.setValue(this.plugin.settings.enableToolUse).onChange(async value => {
    ...
    new Setting(containerEl)
      ...
      .addToggle(toggle =>
        toggle.setValue(this.plugin.settings.fixBrokenEmojis).onChange(async value => {
    ...
    new Setting(containerEl)
      .setName("Max Document Content Length for LLM Prompt (Characters)")
      .setDesc(
        "Limits characters from attached text/markdown documents sent directly in the LLM prompt (0=no limit). Does not affect RAG."
      )
      .addText(text =>
        text
          .setPlaceholder(String(DEFAULT_SETTINGS.maxDocumentContentLengthForLlm))
          .setValue(String(this.plugin.settings.maxDocumentContentLengthForLlm))
          .onChange(async value => {
            const num = parseInt(value.trim(), 10);
            this.plugin.settings.maxDocumentContentLengthForLlm =
              !isNaN(num) && num >= 0 ? num : DEFAULT_SETTINGS.maxDocumentContentLengthForLlm;
            await this.plugin.saveSettings();
          })
      );
    // --- КІНЕЦЬ НОВОГО НАЛАШТУВАННЯ ---
    ...
    if (this.plugin.settings.useAdvancedContextStrategy) {
      ...
      if (this.plugin.settings.enableSummarization) {
        ...
        const updateSummarizationOptions = async (
    ...
    if (this.plugin.settings.translationProvider === "ollama") {
      ...
      const updateOllamaTranslationOptions = async (
        dropdown: DropdownComponent | null,
        button?: ExtraButtonComponent
      ) => {
        if (!dropdown) return;
    ...
    if (this.plugin.settings.fileLoggingEnabled) {
      ...
      new Setting(containerEl)
        ...
        .addText(text => text.setValue(this.plugin.logger.getLogFilePath()).setDisabled(true));
