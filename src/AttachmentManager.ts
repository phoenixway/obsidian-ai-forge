...
export class AttachmentManager {
  ...
  private activeTabId: "images" | "documents" | "links" = "links"; // Зберігаємо ID активної вкладки тут, 'links' за замовчуванням
  ...
  public setActiveTabId(tabId: "images" | "documents" | "links"): void {
    this.activeTabId = tabId;
    // Можна додати логування або іншу логіку, якщо потрібно
  ...
  public getActiveTabId(): "images" | "documents" | "links" {
  ...
  public show(): void {
    ...
    // Передаємо поточний activeTabId з менеджера в конструктор модального вікна
    this.modal = new AttachmentModal(
      this.app,
      this.plugin,
      this, // передаємо сам менеджер
      this.attachmentButtonElement,
      this.getActiveTabId() // <-- Передаємо ID активної вкладки
    );
    this.modal.open();
  }
...
class AttachmentModal extends Modal {
  ...
  constructor(
    app: App,
    plugin: OllamaPlugin,
    manager: AttachmentManager, // ++ manager тепер є параметром
    triggerElement: HTMLElement | null,
    initialActiveTabId: "images" | "documents" | "links"
  ) {
    super(app);
    this.plugin = plugin;
    this.manager = manager; // ++ Зберігаємо менеджер
    this.triggerElement = triggerElement;
    this.activeTab = initialActiveTabId; // Встановлюємо активну вкладку для цього екземпляра модалки
  ...
  public refreshCurrentTabContent(): void {
    if (this.isCurrentlyOpen && this.contentContainerEl) {
      this.renderContent(); // Це викличе рендеринг на основі this.activeTab
      this.positionModal();
    }
  }
  ...
  private createTabButton(label: string, tabId: "images" | "documents" | "links", iconName: string): void {
    ...
    // Визначаємо активність на основі this.activeTab цього екземпляра модалки
    if (this.activeTab === tabId) {
      tabButton.addClass(CSS_ATTACHMENT_TAB_ACTIVE);
    }

    tabButton.onClickEvent((event: MouseEvent) => {
      event.stopPropagation();

      if (this.activeTab === tabId) return;

      this.activeTab = tabId; // Оновлюємо локальну активну вкладку
      this.manager.setActiveTabId(tabId); // Повідомляємо менеджеру

      this.renderTabs(); // Перерендерить таби, використовуючи оновлений this.activeTab
      this.renderContent(); // Перерендерить контент на основі оновленого this.activeTab
      this.positionModal();
    });
  }
  ...
  private renderContent(): void {
    ...
    switch (
      this.activeTab // Використовуємо this.activeTab
    ) {
      case "images":
  ...
  private createFileListDisplay(
    ...
    if (files.length === 0) {
      ...
      files.forEach(item => {
        ...
        removeButton.onClickEvent((event: MouseEvent) => {
          // <-- Додаємо event
