...
export class OllamaView extends ItemView {
  ...
  private lastProcessedChatId: string | null = null; // Ця властивість вже має бути
	...
	public isMenuOpen(): boolean {
		return (
			!!this.menuDropdown && this.menuDropdown.style.display === "block"
		);
	}
    ...
    // flexContainer.style.display = 'flex'; // Стиль буде в SCSS

    // 1. Панель Ролей (зліва чи справа?) - Давайте зробимо зліва
    ...
    // 2. Основна Область Чату (права частина)
    ...
    // Контейнер вводу (знизу основної області)
    ...
    // ... (кнопка перекладу, дисплей моделі, температури - ЯК БУЛО) ...
    ...
    this.roleDisplayEl = leftControls.createDiv({ cls: CSS_CLASS_ROLE_DISPLAY }); this.roleDisplayEl.setText("..."); this.roleDisplayEl.title = "Click to select role"; // Цей елемент тепер може бути менш важливим на десктопі
    ...
    // ... (кнопки Send, Voice, Menu, Toggle Location - ЯК БУЛО) ...
    ...
    // Випадаюче меню (залишається для інших опцій)
    this.menuDropdown = inputContainer.createEl("div", { cls: [CSS_CLASS_MENU_DROPDOWN, "ollama-chat-menu"] });
    // ... (вміст меню ЯК БУЛО, АЛЕ секцію ролей можна прибрати або залишити для мобільних) ...
    // Можна залишити секцію ролей, стилі її приховають на десктопі
...
/////////
    ...
    private createSubmenuSection = (title: string, icon: string, listContainerClass: string, sectionClass?: string): { header: HTMLElement, content: HTMLElement, section: HTMLElement } => {
	...
	private attachEventListeners(): void {
		...
		if (this.toggleLocationButton) {
			...
			); // Використовуємо той самий обробник
		} else {
		...
		if (this.settingsOption)
			this.settingsOption.addEventListener(
				"click",
				this.handleSettingsClick
			);
		else console.error("settingsOption missing!");
		if (this.clearChatOption)
			this.clearChatOption.addEventListener(
				"click",
		...
		else console.error("exportChatOption missing!"); // Обробник залишається той самий
		if (this.newChatOption)
			this.newChatOption.addEventListener(
				"click",
		...
		if (this.cloneChatOption)
			this.cloneChatOption.addEventListener(
				"click",
		...
		this.registerDomEvent(
			document,
			"click",
			this.handleDocumentClickForMenu
		);
		this.registerDomEvent(
			document,
			"visibilitychange",
...
private updateRolePanelList = async (): Promise<void> => {
    ...
    try {
        ...
        // --- Опція "None" ---
        const noneOptionEl = this.rolePanelListEl.createDiv({ cls: [CSS_ROLE_PANEL_ITEM, CSS_ROLE_PANEL_ITEM_NONE, 'menu-option'] }); // Додаємо menu-option для перевикористання стилів
        ...
        this.registerDomEvent(noneOptionEl, 'click', () => this.handleRolePanelItemClick(null, currentRolePath)); // Передаємо null як RoleInfo
...
private handleRolePanelItemClick = async (roleInfo: RoleInfo | null, currentRolePath: string | null | undefined): Promise<void> => {
	...
	private handleModelDisplayClick = async (event: MouseEvent) => {
		...
		try {
							...
							.onClick(async () => {
								...
								if (modelName !== latestModelName) {
									if (chatToUpdate) {
										...
										new Notice(
											"Cannot set model: No active chat."
										);
									}
								}
							})
			...
			loadingNotice.hide();
			console.error(
				"Error loading models for model selection menu:",
				error
			);
			menu.addItem((item) =>
			...
			if (itemsAdded) {
				...
				console.warn(
					"Model menu was not shown because no items were added."
				);
	...
	private handleTranslateInputClick = async (): Promise<void> => {
		...
		try {
			const translatedText =
				await this.plugin.translationService.translate(
					currentText,
					targetLang
				);
			...
			this.translateInputButton.classList.remove(
				CSS_CLASS_TRANSLATING_INPUT
			);
	...
	private collapseAllSubmenus(exceptContent?: HTMLElement | null): void {
		...
		submenus.forEach((submenu) => {
			...
			if (
				...
				if (
					!submenu.content.classList.contains(
						CSS_CLASS_SUBMENU_CONTENT_HIDDEN
					)
				) {
					submenu.content.classList.add(
						CSS_CLASS_SUBMENU_CONTENT_HIDDEN
					);
	...
	private handleExportChatClick = async (): Promise<void> => {
		...
		try {
			...
			const safeName = activeChat.metadata.name.replace(
				/[\\/?:*"<>|]/g,
				"-"
			); // Make filename safe
			...
			let targetFolderPath =
				this.plugin.settings.chatExportFolderPath?.trim();
			...
			if (targetFolderPath) {
				...
				if (!abstractFile) {
					try {
						...
						if (targetFolder) {
							new Notice(
								`Created export folder: ${targetFolderPath}`
							);
						} else {
							...
							new Notice(
								`Error creating export folder. Saving to vault root.`
							);
							targetFolder = this.app.vault.getRoot();
						}
					} catch (err) {
						this.plugin.logger.error(
							"Error creating export folder:",
							err
						);
						new Notice(
							`Error creating export folder. Saving to vault root.`
			...
			if (!targetFolder) {
				...
				new Notice(
					"Error determining export folder. Cannot save file."
				);
				return;
			}
	...
	private handleChatListUpdated = (): void => {
		// Original console log: console.log("[OllamaView] Chat list updated event received.");
		this.plugin.logger.info(
			"[handleChatListUpdated] Received 'chat-list-updated' event."
		); // Log event reception

		const menuOpen = this.isMenuOpen(); // Check menu state
		this.plugin.logger.debug(
			`[handleChatListUpdated] Is menu open? ${menuOpen}`
		);

		if (menuOpen) {
			// Check if the chat submenu is actually expanded
			...
			if (isChatSubmenuVisible) {
				this.plugin.logger.info(
					"[handleChatListUpdated] Menu and chat submenu are open, calling renderChatListMenu()."
				);
				// Calling renderChatListMenu is async, log before calling
				this.renderChatListMenu(); // Let renderChatListMenu log its own start/end
			} else {
				this.plugin.logger.debug(
					"[handleChatListUpdated] Menu is open, but chat submenu is collapsed. Not re-rendering list."
				);
			}
		} else {
			this.plugin.logger.debug(
				"[handleChatListUpdated] Menu is closed. Not re-rendering list."
			);
		}
	};
...
private handleActiveChatChanged = async (data: { chatId: string | null, chat: Chat | null }): Promise<void> => {
    this.plugin.logger.debug(`[OllamaView] Active chat changed event received. New ID: ${data.chatId}, Previous ID: ${this.lastProcessedChatId}`);
    ...
    if (chatSwitched) {
        // --- ЗМІНИВСЯ ЧАТ або став NULL ---
        this.plugin.logger.info(`[handleActiveChatChanged] Chat switched from ${previousChatId} to ${data.chatId}. Reloading view.`);
        // !!! ВИКЛИКАЄМО БЕЗ АРГУМЕНТІВ (ВИПРАВЛЕНО) !!!
        await this.loadAndDisplayActiveChat(); // Лінія ~1711
        ...
        // НЕ викликаємо loadAndDisplayActiveChat тут
    } else {
         // Неочікуваний випадок (chatId не змінився, але став null АБО data.chat === null)
         // Або випадок, коли chatId став null (вже оброблено в першому if) - уточнимо логіку
         if (data.chatId === null) {
              this.plugin.logger.info(`[handleActiveChatChanged] Active chat explicitly set to null. Reloading view.`);
         } else { // chatId не null, але data.chat === null
         ...
         // !!! ВИКЛИКАЄМО БЕЗ АРГУМЕНТІВ (ВИПРАВЛЕНО) !!!
         await this.loadAndDisplayActiveChat(); // Лінія ~1739 (приблизно, після реструктуризації)
    }

    // Оновлення меню (залишається без змін)
    if (this.isMenuOpen() && this.roleSubmenuContent && !this.roleSubmenuContent.classList.contains(CSS_CLASS_SUBMENU_CONTENT_HIDDEN)) {
	...
	private updateSendButtonState(): void {
		...
		const isDisabled =
			this.inputEl.value.trim() === "" || this.isProcessing;
	...
	public setLoadingState(isLoading: boolean): void {
		...
		if (this.translateInputButton) {
			...
			this.translateInputButton.classList.toggle(
				CSS_CLASS_DISABLED,
				isLoading
			);
		}
  ...
  // --- ВАЖЛИВО: Переконайтесь, що цей метод також оновлений! ---
	...
	async sendMessage(): Promise<void> {
		...
		try {
			...
			const assistantMessage =
				await this.plugin.ollamaService.generateChatResponse(
					activeChat
				);
	...
	private renderMessageInternal(
		...
		switch (message.role) {
			...
			case "system":
				setIcon(
					contentEl.createSpan({ cls: CSS_CLASS_SYSTEM_ICON }),
					"info"
				);
	...
	private async handleRegenerateClick(userMessage: Message): Promise<void> {
		...
		if (messageIndex === activeChat.messages.length - 1) {
			new Notice(
				"This is the last message, nothing to regenerate after it."
			);
			return;
		}
		...
		new ConfirmModal(
			...
			async () => {
				...
				try {
					...
					const updatedChat =
						await this.plugin.chatManager.getActiveChat(); // Отримати актуальну версію
					if (!updatedChat)
						throw new Error(
							"Failed to get updated chat after deletion."
						);
					...
					this.plugin.logger.error(
						"Error during regeneration process:",
						error
					);
	...
	private handleCopyClick(content: string, buttonEl: HTMLElement): void {
		...
		if (
			this.detectThinkingTags(this.decodeHtmlEntities(content))
				.hasThinkingTags
		) {
	...
	private async handleTranslateClick(
		...
		contentEl
			.querySelector(`.${CSS_CLASS_TRANSLATION_CONTAINER}`)
			?.remove();
		...
		contentEl.querySelectorAll("pre").forEach((pre) => {
			...
			if (langClass) {
				...
				if (lang) {
					...
					if (
						!pre.querySelector(`.${CSS_CLASS_CODE_BLOCK_LANGUAGE}`)
					) {
						pre.createEl("span", {
			...
			this.mediaRecorder.onstop = () => {
				...
				if (this.speechWorker && audioChunks.length > 0) {
					...
					this.speechWorker.postMessage({
						...
						audioBlob,
						languageCode:
							this.plugin.settings.speechLanguage || "uk-UA",
					});
	...
	private checkMessageForCollapsing(messageEl: HTMLElement): void {
		...
		if (!c || h <= 0) {
			...
			if (c && h <= 0) {
				const b = messageEl.querySelector(
					`.${CSS_CLASS_SHOW_MORE_BUTTON}`
				);
				b?.remove();
				c.style.maxHeight = "";
	...
	private handleRoleDisplayClick = async (event: MouseEvent) => {
		...
		try {
			...
			// loadingNotice?.hide();
			console.error(
				"Error loading roles for role selection menu:",
				error
			);
		...
		new PromptModal(
			...
			async (newValue) => {
				...
				try {
					...
					this.plugin.logger.error(
						"Failed to update chat temperature:",
						error
					);
	...
	private updateToggleViewLocationOption(): void {
		...
		if (this.plugin.settings.openChatInTab) {
			...
			this.toggleViewLocationOption.title =
				"Close tab and reopen in sidebar";
		} else {
